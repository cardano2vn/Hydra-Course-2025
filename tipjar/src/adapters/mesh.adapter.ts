...
import { applyParamsToScript, deserializeAddress, IFetcher, MeshTxBuilder, MeshWallet, mPubKeyAddress, PlutusScript, scriptAddress, serializeAddressObj, serializePlutusScript, UTxO } from "@meshsdk/core";
...
export class MeshAdapter {
    ...
    constructor({ meshWallet = null!, owner, minimumTip }: { meshWallet: MeshWallet, owner: string, minimumTip: number }) {
        ...
        this.spendAddress = serializeAddressObj(scriptAddress(deserializeAddress(serializePlutusScript(this.spendScript, undefined, APP_NETWORK_ID, false).address).scriptHash, "", false), APP_NETWORK_ID)
    }
