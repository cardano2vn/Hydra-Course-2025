...
import { applyParamsToScript, deserializeAddress, IFetcher, mConStr0, mConStr2, mConStr3, MeshTxBuilder, MeshWallet, mPubKeyAddress, PlutusScript, scriptAddress, serializeAddressObj, serializePlutusScript, UTxO } from "@meshsdk/core";
...
export class HydraAdapter {
    ...
    constructor({ meshWallet, hydraProvider, owner, minimumTip}: { meshWallet: MeshWallet, hydraProvider: HydraProvider, owner: string, minimumTip: number}) {
        ...
        this.spendAddress = serializeAddressObj(scriptAddress(deserializeAddress(serializePlutusScript(this.spendScript, undefined, APP_NETWORK_ID).address).scriptHash, "", false), APP_NETWORK_ID)
    }
