...
# Contracts
cd contract
aiken check                   # Syntax check + run tests
aiken check -m <test_name>    # Run test matching name
aiken build                   # Compile to Plutus v3
aiken docs                    # Generate HTML docs (if lib)

# Utilities
jq '.validators[] | .title' plutus.json    # List validators
ls build/default/                           # View compiled artifacts
```
...
- ✅ Tạo các file mẫu (`samples/alice_utxo.json`, `.gitignore`, `scripts/build-and-test.sh`).
- ✅ Viết một validator Aiken demo (escrow / mint token) thay thế `placeholder.ak`.
- ✅ Viết unit test cho contract.

**Trả lời**: "Tạo file mẫu", "Viết contract demo", hoặc cả hai, tôi sẽ áp dụng vào repo ngay!
...
- [Hydra Official](https://hydra.family)
- [Cardano Node Releases](https://github.com/IntersectMBO/cardano-node)
